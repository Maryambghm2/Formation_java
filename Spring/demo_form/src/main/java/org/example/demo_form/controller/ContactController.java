...
public class ContactController {
    ...
    public String showContact(@RequestParam(value = "name", required = false) String name, Model model) {
        ...
        if (contact != null) { // si j'ai bien trouver un contact
            model.addAttribute("contact", contact);
            return "pagec";
        } else { // si je n'ai pas trouve de contact je renvoie vers la route /list
    ...
    // Endpoint pour récupérer les donnees envoyer par le formulaire avec method Get
    @GetMapping("/add")
    public String recupFormGet(@RequestParam("name") String name,
        ...
        // Endpoint pour afficher la page formulaire (formulaire avec method Post)
        @GetMapping("/formpost")
        ...
        model.addAttribute("contact", new Contact());
            ...
            // Endpoint pour récupérer les données envoyer par le formulaire avec methode post
@PostMapping("/add")
